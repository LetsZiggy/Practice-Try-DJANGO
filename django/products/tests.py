...
def create_product(title="", description="", price=0, summary="", featured=True):
  return Product.objects.create(
    title=title, description=description, price=price, summary=summary, featured=featured
  )
...
class ProductsDetailViewTests(TestCase):
  def test_detail_page(self):
    item = create_product(
      title="item 01", price=1.01, summary="Item 01", description="Description 01"
    )
...
class ProductsCreateViewTests(TestCase):
  def test_valid_create_page(self):
    item = create_product(
      title="item 01", description="", price=1.01, summary="Item 01", featured=True
    )
    data = {
      "title": item.title,
...
class ProductsListViewTests(TestCase):
  ...
  def test_list_page_with_products(self):
    ...
    self.assertTemplateUsed(response, "products/product_list.html")
    self.assertTrue(
      re.search(r'<a href="/products/detail/\d+/">[\s\n]+Name: Item \w+[\s\n]+</a>', content)
    )
